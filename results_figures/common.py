...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "results_figures_v1"
...
def save_figure_with_svg_copy(fig: plt.Figure, filename: str) -> Path:
    ensure_output_dir()
    primary_path = OUTPUT_DIR / filename
    svg_path = primary_path.with_suffix(".svg")
    fig.savefig(primary_path, bbox_inches="tight", facecolor="white", dpi=300)
...
def create_threshold_candidate_violin_plot(
    threshold: float = 426.90,
    optimization_dir: Path | None = None,
    filename: str = "fig07_candidates_above_threshold_violin.svg",
) -> Path:
    ...
    return save_figure_with_svg_copy(fig, filename)
...
def create_fig01_correlation_heatmap() -> Path:
    ...
    return save_figure(fig, "fig01_correlation_heatmap.svg")
...
def create_fig01_feature_temperature_energy_boxplot() -> Path:
    ...
    plt.tight_layout()
    return save_figure(fig, "fig01_feature_temperature_energy_boxplot.svg")
...
def create_fig13_parameter_response_network() -> Path:
    ...
    plt.tight_layout()
    return save_figure(fig, "fig13_parameter_response_network.svg")
...
def create_fig02_model_comparison() -> Path:
    ...
    return save_figure(fig, "fig02_model_comparison.svg")
...
def create_fig12_temperature_parity_train() -> Path:
    ...
    return _create_temperature_parity_figure(
        ...
        "fig12_temperature_parity_train.svg",
    )
...
def create_fig04_ablation() -> Path:
    ...
    return save_figure(fig, "fig04_ablation.svg")
...
def create_fig05_expert_loss_band() -> Path:
    ...
    return save_figure(fig, "fig05_expert_loss_band.svg")
...
def create_fig06_case_overview() -> Path:
    ...
    for col_idx, case in enumerate(selected_cases):
        ...
        if col_idx == 0:
            temp_ax.set_ylabel("Outlet temperature (°C)", fontsize=16)
        ...
        energy_ax.set_xlabel("Time (h)", fontsize=18)
        if col_idx == 0:
            energy_ax.set_ylabel("Cumulative energy (MJ)", fontsize=18)
    ...
    fig.legend(handles, labels, ncol=5, loc="upper center", bbox_to_anchor=(0.5, 0.99), fontsize=18, handlelength=2.4, columnspacing=1.2)
    fig.subplots_adjust(top=0.90, bottom=0.11, wspace=0.16, hspace=0.14)
    return save_figure_with_svg_copy(fig, "fig06_case_overview.png")
...
def create_fig07_optimization_landscape() -> Path:
    ...
    return save_figure(fig, "fig07_optimization_landscape.svg")
...
def create_fig08_optimization_benefit() -> Path:
    ...
    return save_figure(fig, "fig08_optimization_benefit.svg")
...
def create_fig09_expert_boxplot() -> Path:
    ...
    ax.grid(True, axis="y")
    ax.grid(False, axis="x")
    return save_figure(fig, "fig09_expert_boxplot.svg")
...
def create_fig10_parameter_shift() -> Path:
    ...
    return save_figure(fig, "fig10_parameter_shift.svg")
...
def create_fig11_training_loss() -> Path:
    ...
    plt.tight_layout()
    return save_figure(fig, "fig11_training_loss.svg")
